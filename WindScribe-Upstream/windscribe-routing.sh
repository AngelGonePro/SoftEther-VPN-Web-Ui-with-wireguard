...
# Host route: SoftEther replies via vpn-net bridge (use Docker network ID)
...
# SoftEther inbound connection routing fix
# Ensures replies to inbound VPN connections go directly, not through Windscribe
SE_IP=$(docker inspect softether --format \
    '{{(index .NetworkSettings.Networks "softether-vpn_vpn-net").IPAddress}}' 2>/dev/null)
if [ -n "$SE_IP" ]; then
    # Set SoftEther default route through windscribe-gw for outbound client traffic
    docker exec softether ip route replace default via 172.21.0.2 dev eth0 2>/dev/null || true
    # Direct route table for inbound replies
    ...
    # Mark inbound packets destined for SoftEther
    ...
    # Mark outbound replies from SoftEther VPN ports to go direct
